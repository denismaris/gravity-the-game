...
export interface TowersPuzzle {
  ...
  /** See `PuzzleDifficulty`'s own comment. This game's difficulty comes
   * from `size` plus which clue sides are dropped (see `puzzles.ts`'s own
   * comment), not a stored numeric rating - assigned by hand per puzzle
   * rather than derived, since "which sides are dropped" isn't a single
   * scalar a formula could read off cleanly. */
